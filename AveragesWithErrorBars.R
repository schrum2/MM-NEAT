...
setwd("C:\\Users\\schrum2\\eclipse-workspace\\MM-NEAT\\cartpole")
rm(list = ls())
...
png("cartpole.png", width=2000, height=1000)
v <- ggplot(evolutionStats, aes(x = generation, y = avgScore, color = type)) +
  ...
  geom_line(size = 1.5) + # + facet_wrap(~type)
  ggtitle("Evolution in Cartpole") +
  ylab("Average Score") +
  theme(
