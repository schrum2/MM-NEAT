...
public class TransposTable {
	...
	public static void clear() {
		...
		++currentVersion;
		/*
		 * for (int i = 0; i < table.length; ++i) { table[i].hash = 0; } for
		 * (int i = 0; i < cachedStaticInfo.length; ++i) {
	...
	public static boolean retrieve(Board b, PlyInfo p, boolean movePacman) {
		...
		if (t.hash == hash && t.version == currentVersion) {
			...
			if (t.pacmanLocation != b.pacmanLocation || movePacman != ((t.flags & MOVE_PACMAN) != 0)) {
				return false;
			}
			...
			if (willDie && p.budget <= 0) {
				// death evaluations made with budget < 0 are not 100% reliable
				// and will
				// propagate upwards; we trust them only a limited time.
				willDie = t.budget >= p.budget - 20;
			}
			if (t.budget >= p.budget || willDie) {
				if (kind == REAL_VALUE || (kind == UPPER_BOUND && t.value <= p.alpha)
						|| (kind == LOWER_BOUND && t.value > p.beta) || willDie) {
	...
	static class CachedStaticInfo {
		...
		public String toString() {
			return "CachedStaticInfo [hash=" + hash + ", nrBorders=" + nrBorders + ", nrPacmanNodes=" + nrPacmanNodes
					+ ", hasCircles=" + hasCircles + ", matchCalled=" + matchCalled + ", matchResult=" + matchResult
					+ ", canReachPowerPill=" + canReachPowerPill + ", bestNrAssignedGhosts=" + bestNrAssignedGhosts
					+ ", ghostAssignCalled=" + ghostAssignCalled + ", nrInvolvedGhosts=" + nrInvolvedGhosts
