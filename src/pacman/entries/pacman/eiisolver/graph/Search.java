...
public class Search {
	...
	public static void searchMove(Game newGame, long timeDue) {
		...
		emergencyStopTime = startTime + 100000; // we want to search at least 1
												// ply without emergency stops
		...
		while (!stop) {
			...
			emergencyStopTime = timeDue - 7; // now set the real emergency stop
												// time, with a little slack.
			long timeSpent = System.currentTimeMillis() - startTime;
			stop = Math.abs(p.bestValue) >= PACMAN_WILL_DIE || startTime + timeSpent >= normalStopTime
		...
		if (emergencyStopped) {
			...
			// search was stopped in the middle of a ply, cannot use search
			// result
			...
			// in our backup we did not loose, but in the last search we
			// discovered that we
			// will loose anyway. We try to make it as hard as possible for the
			// opponent,
	...
	public static void search() {
		...
		if (log) {
			log("search " + currDepth + ", nodes = " + nodesSearched + " [" + p.alpha + ", " + p.beta + "], budget = "
					+ p.budget);
		}
		...
		if (evenPly) {
			...
			if (extendedSearchDepth > 0) {
				if (p.budget <= -60 || currDepth >= MAX_PLY - 4) {
					// end of extended search reached; static check if pacman is
					// in danger
				...
				if (currDepth < 2 || plyInfo[currDepth - 2].nrPossibleMoves > 1
						|| plyInfo[currDepth - 1].nrPossibleMoves > 1) {
					...
					// If pacman is totally safe a few moves before the end we
					// return before reaching budget 0.
					if (currDepth > 20 && p.budget <= 30) {
						dropOutEarly = staticEval2.nrBorders >= 8
								|| (staticEval2.nrBorders > 6 && staticEval2.hasCircles);
					}
					if (log && dropOutEarly) {
						log("drop out early");
					}
		...
		if (p.nrPossibleMoves > 1) {
			if (TransposTable.retrieve(b, p, movePacman)) {
				if (log) {
					if (movePacman) {
						log("From transpos: pacman move, " + nodes[b.pacmanLocation].neighbourMoves[p.bestPacmanMove]
								+ ", value: " + p.bestValue);
					} else {
		...
		while (!cutoff && p.nextMove(movePacman)) {
			...
			nextP.alpha = -p.beta;
			nextP.beta = -((p.alpha) > (p.bestValue) ? (p.alpha) : (p.bestValue));
		...
		if (extendedSearchDepth == currDepth) {
			...
			if (ghostTargets[0][0].nrBackups > 0 || p.budget > 0) {
				...
				// throw new RuntimeException("target.backups = " +
				// ghostTargets[0][0].nrBackups);
			}
		}
	}
	...
	private static boolean evaluate(PlyInfo p, boolean movePacman, boolean mustTakePowerPillToSurvive) {
		...
		if (!pacmanCanGetToPowerPill || !pacmanEvaluation || heuristics.getPowerPillScore() < 0) {
			...
			if (staticEval2.nrBorders < 8 && staticEval2.nrPacmanNodes < 8) {
				 ...
				 * staticEval2.match(); if (staticEval2.match() &&
				 * pacmanCanGetToPowerPill) {
				...
				/*
				 * if (staticEval2.match()) { if (staticEval2.canReachPowerPill)
				 * { graphBonus += heuristics.getPowerPillScore()/4; } else {
				 * graphBonus -= 1000; } }
				 */
			} else {
				if (!pacmanEvaluation) {
		...
		for (MyGhost ghost : b.ghosts) {
			if (ghost.lairTime == 0) {
				// int dist =
				// graph.getGhostDistToJunction(ghost.currentNodeIndex,
				// ghost.lastMoveMade, nextPacmanJunction.index,
				// nextPacmanJunction.neighbourMoves[0]);
		...
		// calculate shortest path to eat all edible ghosts, assuming pacman is
		// greedy;
		...
		if (nrEdibleGhosts > 0) {
			...
			/*
			 * if (pacmanEvaluation) { // pacman will move to closest edible
			 * ghost int shortestDist = 500; int nearestGhost = -1; for (int g =
			 * 0; g < nrEdibleGhosts; ++g) { int dist =
			 * game.getShortestPathDistance(b.pacmanLocation, edibleGhosts[g]);
			 * dist += dist/4; // assume ghost moves away if (dist <
			 * shortestDist) { boolean killingGhostIsCloser = false; for
			 * (MyGhost ghost : b.ghosts) { if (ghost.canKill()) { if
			 * (game.getShortestPathDistance(ghost.currentNodeIndex,
			 * edibleGhosts[g]) < shortestDist) { if(log)log(
			 * "killing ghost is closer "); killingGhostIsCloser = true; break;
			 * } } } if (killingGhostIsCloser) { shortestDist += 30; } if (dist
			 * < shortestDist) { shortestDist = dist; nearestGhost = g; } } if
			 * (dist > longestDist) { longestDist = dist; } } if(log)log(
			 * "nearest ghost: " + shortestDist + ", maxEdible = " +
			 * maxEdibleTime); if (shortestDist < maxEdibleTime) { edibleBonus =
			 * 10*GHOST_EAT_SCORE-25*shortestDist; if(log)log("edible bonus: " +
			 * edibleBonus); }
			 * 
			 * } else {
			 */
				...
				for (MyGhost ghost : b.ghosts) {
					if (ghost.edibleTime == 0) {
						...
						if (ghost.lairTime == 0) {
							...
							dist = ghost.lairTime
									+ game.getShortestPathDistance(game.getGhostInitialNodeIndex(), edibleGhosts[g]);
						}
			...
			int pathLength = 0; // will contain the length of the path that eats
								// all ghosts
				...
				if (true || pacmanEvaluation) {
					...
					for (int i = 0; i < nrGhostsInRange; ++i) {
						...
						if (pathLengths[i] <= threshold) {
							...
							bonus = GHOST_EAT_SCORE + ((maxScore - GHOST_EAT_SCORE) * (pathLengths[i] - threshold))
									/ (maxEdibleTime - threshold) - 2 * pathLengths[i];
						}
						if (bonus >= 0) {
							edibleBonus += bonus;
						}
						if (log) {
							log("g: " + i + ", bonus: " + bonus + ", pathLen: " + pathLengths[i] + ", total: "
									+ edibleBonus);
						}
					}
					// edibleBonus = 6*GHOST_EAT_SCORE*nrGhostsInRange -
					// 10*distToGhost1 -5*pathLength ;
				} else {
					edibleBonus = 1000 - 5 * distToGhost1 - 5 * pathLength + 300 * nrGhostsInRange;
				}
			}
			if (log) {
				log("pathLenght: " + pathLength + ", nrGhostsInRange: " + nrGhostsInRange + ",edibleBonus: "
						+ edibleBonus + ", distToGhost1: " + distToGhost1 + ", pathLen: " + pathLength
						+ ", edibleTime: " + maxEdibleTime);
			}
			// }
		}
		...
		if (pacmanEvaluation) {
			if (heuristics.hasManyLivesLeft()) {
				...
				if (heuristics.isWeakOpponent()) {
					...
					if (edibleBonus == 0) {
						value -= farAwayBonus; // we want pacman to be close to
												// all ghosts if they are weak
					}
				}
			} else {
				value += 4 * graphBonus;
			...
			/*
			 * if (nrInLair >= 3 && nrEdibleGhosts == 0 &&
			 * game.getCurrentLevel() == 0) { System.out.println("nrlair");
			 * value = 80*(60-game.getShortestPathDistance(b.pacmanLocation,
			 * game.getGhostInitialNodeIndex())); }
			 */
			value += rand.nextInt(4);
		} else {
			value = graphBonus + 5 * closestDist + farAwayBonus + rand.nextInt(5) + (p.score + edibleBonus) / 8;
			if (heuristics.isWeakOpponent()) {
				if (nrEdibleGhosts == 0) {
					if (pacmanCanGetToPowerPill) {
		...
		if (log) {
			log("eval: value = " + p.bestValue + ", graphBonus: " + graphBonus + ", far: " + farAwayBonus + ", close: "
					+ closestDist + (edibleBonus != 0 ? "" : ", edible: " + edibleBonus) + ", score: " + p.score);
		}
		return extendSearch;
	}
	...
	private static int killingGhostEvaluation() {
		...
		// the closer pacman is to the pill, the further away we want the ghosts
		// to be.
		// Preferably the ghosts are out of edible range. But 1 ghost needs to
		// be close
		...
		return value;
		// if(log)log("killingGhosts: farAwayFee = " + farAwayFee + ",
	 ...
	 *         pill, or a high value if pacman will die (the higher value, the
	 *         less time it will take for the ghosts to capture pacman)
	 */
	public static int checkPacmanHealth(PlyInfo p, boolean movePacman) {
		...
		if (staticEval2.wouldDieWithoutPowerPill()) {
			pacmanCanGetToPowerPill = staticEval2.canReachPowerPill;
			if (pacmanCanGetToPowerPill) {
				// pacman would normally die, but fortunately it can reach a
				// power pill
				if (log) {
			...
			// pacman will die!
			// try to make it as difficult for the ghosts as possible to find
			// the correct path.
			int longestDist = 0;
		...
		/*
		 * boolean pacmanDies = false; int difficulty = 0; if
		 * (pacmanNode.isJunction()) { difficulty = 10;
		 * staticEvaluator.nrJunctions = pacmanNode.nrNeighbours; for (int i =
		 * 0; i < pacmanNode.nrNeighbours; ++i) { BigEdge edge =
		 * nodes[pacmanNode.neighbours[i]].edge; Node otherJunction =
		 * edge.getOtherJunction(pacmanNode); staticEvaluator.edges[i] = edge;
		 * staticEvaluator.junctions[i] = otherJunction.index;
		 * staticEvaluator.viaJunctions[i] = otherJunction.index; } pacmanDies =
		 * staticEvaluator.checkPacmanHealth("pacman on junction"); } else { //
		 * check if there are two other ghosts on the same edge as pacman,
		 * attacking from // both sides difficulty = 0; BigEdge pacmanEdge =
		 * pacmanNode.edge; pacmanDies =
		 * staticEvaluator.checkPacmanHealth(pacmanEdge.endpoints[0].index,
		 * pacmanEdge, pacmanEdge.endpoints[1].index, pacmanEdge,
		 * "direct pacman edge"); if (!pacmanDies) { difficulty = 40; pacmanDies
		 * = checkPacmanEdgeJunction(pacmanEdge.endpoints[0].index); } if
		 * (!pacmanDies) { pacmanDies =
		 * checkPacmanEdgeJunction(pacmanEdge.endpoints[1].index); } if
		 * (!pacmanDies && pacmanEdge.endpoints[0].nrNeighbours +
		 * pacmanEdge.endpoints[1].nrNeighbours <= 6) { // pacman will die if
		 * ghosts are closer to the 4 junctions that are 2 junctions away from
		 * pacman difficulty = 60; int nrJunctions = 0; for (int e = 0; e < 2;
		 * ++e) { Node junction = pacmanEdge.endpoints[e]; for (int i = 0; i <
		 * junction.nrNeighbours; ++i) { BigEdge edge = junction.edges[i]; if
		 * (edge != pacmanEdge) { // otherJunction lies 2 junctions away Node
		 * otherJunction = edge.getOtherJunction(pacmanEdge.endpoints[e]);
		 ...
		 * ++nrJunctions; } } } staticEvaluator.nrJunctions = nrJunctions;
		 * pacmanDies = staticEvaluator.checkPacmanHealth(
		 * "pacman 2 junctions away"); } } return pacmanDies ? PACMAN_WILL_DIE +
		 * 2000 - difficulty - currDepth : 0;
		 */
	}
	...
	private static int _feast(boolean movePacman) {
		...
		for (MyGhost ghost : b.ghosts) {
			if (ghost.lairTime == 0) {
				...
				int distance = game.getShortestPathDistance(b.pacmanLocation, ghost.currentNodeIndex);
				...
				if (ghost.canKill()) {
					...
					if (ghostNode.edge == edge && ghostNode.isOnPath(destNode, ghost.lastMoveMade)) {
						...
						if (newDist < pacmanDist) {
							// the move is towards the ghost; check if there is
					...
					// edible ghost is on the path; don't skip under any
					// condition
			...
			if (b.containsPowerPill[powerPill]) {
				...
				for (MyGhost ghost : b.ghosts) {
					...
					if (ghost.lairTime == 0) {
						...
						dist = ghost.lairTime
								+ game.getShortestPathDistance(game.getGhostInitialNodeIndex(), powerPill);
					}
					if (dist < ghostDist) {
						ghostDist = dist;
					}
				...
				if (pacmanDist + EAT_DISTANCE < ghostDist) {
					// we have found a power pill that is closer to pacman than
					// to any ghost.
					// pacman is safe.
		...
		public boolean checkPacmanHealth(String logMsg) {
			...
			for (int i = 0; i < nrJunctions; ++i) {
				if (pacmanNode.isJunction()) {
					...
					pacmanDist[i] = pacmanNode.edge.getDistanceToJunction(pacmanNode, nodes[viaJunctions[i]])
							+ EAT_DISTANCE;
			...
			for (int j = 0; j < nrJunctions; ++j) {
				...
				for (int g = 0; g < b.ghosts.length; ++g) {
					...
					if (ghost.lairTime == 0) {
						Node ghostNode = nodes[ghost.currentNodeIndex];
						if (ghostNode.edge == edges[j]
								&& ghostNode.getNextJunction(ghost.lastMoveMade) != junctions[j]) {
							// ghost is already on the edge, on the move to
							// pacman. Pacman cannot escape via this junction
							// unless ghost is on same edge as pacman and
							// already past pacman.
							if (pacmanNode.edge != ghostNode.edge
									|| ghostNode.isOnPath(pacmanNode, ghost.lastMoveMade)) {
								ghostIsCloser[g] |= 1 << j;
							...
							int dist1 = ghostDist(ghost,
									game.getShortestPathDistance(ghost.currentNodeIndex, junctions[j]));
							if (dist1 <= pacmanDist[j]) {
								// ghost is closer, but can it also move to the
								// junction
								int dist = ghostDist(ghost, graph.getGhostDistToJunction(ghost.currentNodeIndex,
										ghost.lastMoveMade, junctions[j], firstMoveFromJunctions[j]));
								if (dist <= pacmanDist[j]) {
									...
									if (log) {
										log(j + ": ghost " + ghostNode + " shortest dist = " + dist1 + ", real dist: "
												+ dist);
									}
								}
							}
			...
			// we have some ghost closer to every junction. But it must be
			// distinct ghosts! (one ghost can only cover 1 junction)
	...
	private static void setExtendedSearchTargets() {
		...
		for (int i = 0; i < ghostTargets.length; ++i) {
			...
			if (assignedBorder >= 0) {
				...
				// if (ghost.lairTime == 0) {
				// ghost is not yet on the assigned border edge, so create a
				// target
				Target target = ghostTargets[i][0];
				target.set(edge, i);
				// if already on edge, target is already reached. By adding this
				// target anyway,
				// pacman gets the choice of following pacman or pursuing some
				// other target.
				...
				for (int i = 0; i < ghostTargets.length; ++i) {
					if ((edge.closerGhosts & (1 << i)) != 0) {
						...
						if (ghost.lairTime > 0 || nodes[ghost.currentNodeIndex].edge != edge.edge) {
							// ghost is not yet on the assigned border edge, so
							// create a target
		...
		// determine dead ends (ghosts that have only 1 target: don't walk into
		// those) */
		nrDeadEdges = 0;
		for (int i = 0; i < ghostTargets.length; ++i) {
			if (nrGhostTargets[i] == 1) {
				...
				if (!target.edge.containsPowerPill) {
					...
					// (we know that ghost i will for sure move to this target,
					// so other ghosts do
						...
						if (j != i && nrGhostTargets[j] > 1) {
							for (int k = 1; k < nrGhostTargets[j]; ++k) {
								if (ghostTargets[j][k] == target) {
									if (log) {
										log("Removed target " + target.ghostJunction + " from ghost " + j
												+ " (covered by " + i + ")");
									}
									--nrGhostTargets[j];
	...
	public static class StaticEvaluator2 {
		...
		public void expand() {
			...
			if (n.isJunction()) {
				...
				for (int j = 0; j < 2; ++j) {
					BorderEdge borderEdge = createEdge(n.edge.endpoints[1 - j], n.edge,
							n.edge.getDistanceToJunction(n, n.edge.endpoints[j]));
					if (borderEdge != null) {
						borderEdge.parent = null;
						added[nrAdded] = borderEdge;
						++nrAdded;
				...
				if (borderEdge.closerGhosts == 0) {
					...
					if (existingNode == null) {
						...
						if (borderEdge.edge.containsPowerPill) {
							canReachPowerPill = true; // pacman can reach a
														// power pill
						}
						...
						if (pacmanDistances[existingNode.junctionIndex] > borderEdge.pacmanDist) {
							// node was already present, but now we found a
							// shorter way to it
							pacmanDistances[existingNode.junctionIndex] = borderEdge.pacmanDist;
							expansionNeeded = true; // need to recalculate
													// distances
						}
					}
							...
							if (edge != borderEdge.edge) {
								...
								if (existingEdgeIndex < 0) {
									...
									if (distToJunction < existingEdge.pacmanDist) {
										...
										// if new result is that no ghosts are
										// closer, then this is not a border
										// edge anymore
		...
		private BorderEdge createEdge(Node pacmanJunction, BigEdge edge, int dist) {
			...
			for (int g = 0; g < b.ghosts.length; ++g) {
				...
				if (ghost.lairTime == 0) {
					Node ghostNode = nodes[ghost.currentNodeIndex];
					if (ghostNode.edge == edge
							&& ghostNode.getNextJunction(ghost.lastMoveMade) != otherJunction.index) {
						// ghost is already on the edge, on the move to pacman.
						// Pacman cannot escape via this junction
						// unless ghost is on same edge as pacman and already
						// past pacman.
						Node pacmanNode = nodes[b.pacmanLocation];
						if (pacmanNode.edge != ghostNode.edge || ghostNode.isOnPath(pacmanNode, ghost.lastMoveMade)) {
							boolean closer = ghost.edibleTime == 0;
							if (!closer) {
								// an edible ghost close enough to pacman cannot
								// stop pacman
								...
								if (pacmanNode.edge == ghostNode.edge) {
									...
									ghostToPacmanDist = pacmanDist
											+ ghostNode.getDistToNextJunction(ghost.lastMoveMade);
								}
						...
						if (log) {
							int shortestDist = game.getShortestPathDistance(ghost.currentNodeIndex,
									otherJunction.index);
							if (d < shortestDist) {
								throw new RuntimeException("Internal error in ghostDist, ghostDist = " + d
										+ ", shortest dist = " + shortestDist);
							}
						}
					...
					int ghostDist = ghost.lairTime
							+ graph.getGhostDistToJunction(Search.game.getGhostInitialNodeIndex(), MOVE.NEUTRAL,
									otherJunction.index, borderEdge.firstMoveFromGhost);
					borderEdge.ghostDist[g] = ghostDist;
					if (ghostDist - EAT_DISTANCE <= pacmanDist) {
						borderEdge.closerGhosts |= 1 << g;
		...
		public boolean match() {
			...
			if (nrBorders > 6) {
				...
				// special case, use old static evaluator for edge/junction
				// evaluation, because
				// the new static evaluator cannot handle it correctly if there
				// is for example
				// 1 ghost covering the only 2 border edges, but there are some
				// more ghosts close.
					...
					for (int i = 0; !catchesPacman && i < ghostTargets.length; ++i) {
						if ((unassignedEdge.closerGhosts & (1 << i)) != 0) {
							...
							if (border >= 0) {
								...
								if (statLog) {
									log("unassignedEdge: " + unassignedEdge.edge + ", assignedEdge: "
											+ assignedEdge.edge + ", ghost: " + i);
								}
								path2Length = 0;
								...
								int nrEqual = 0;
								while (nrEqual < path1Length && nrEqual < path2Length
										&& path1[path1Length - nrEqual - 1] == path2[path2Length - nrEqual - 1]) {
									++nrEqual;
								}
								if (statLog) {
									log("path1Length: " + path1Length + ", path2Length: " + path2Length + ", nr equal: "
											+ nrEqual);
								}
								int distToFirst = 0;
								int worstSlack = 1000;
								if (nrEqual > 0) {
									...
									for (int g = 0; g < ghostTargets.length; ++g) {
										...
										if (g2.canKill()) {
											int ghostDist = b.graph.getGhostDistToJunction(g2.currentNodeIndex,
													g2.lastMoveMade, firstNode.index, MOVE.NEUTRAL);
											int slack = ghostDist - 3 - distToFirst;
											if (statLog) {
												log("ghostDist " + g + " to first: " + ghostDist + ", pacmanDist: "
														+ distToFirst + ", worstSlack: " + worstSlack + ", slack: "
														+ slack);
											}
								...
								// unfortunately pacman cannot wait at a
								// junction: must make 1 move and then back.
								// this can influence the slack negatively with
								// 1 (round slack down to even number)
								worstSlack = worstSlack & 0xfffffe;
								int timeUntilPacmanChoice = distToFirst + worstSlack;
								int timeUntilGhostChoice = calcTimeUntilGhostChoice(ghost, assignedEdge,
										unassignedEdge);
								/*
								 * if (timeUntilGhostChoice == 0) { Node
								 * ghostNode = nodes[ghost.currentNodeIndex]; if
								 * (ghostNode.edge != null) {
								 * timeUntilGhostChoice =
								 * ghostNode.getDistToNextJunction(ghost.
								 * lastMoveMade); } }
								 */
								if (statLog) {
									log("ghost choice: " + timeUntilGhostChoice + ", pacman: " + timeUntilPacmanChoice
											+ ",distToFirst: " + distToFirst + ", slack: " + worstSlack);
								}
		...
		private int calcTimeUntilGhostChoice(MyGhost ghost, BorderEdge edge1, BorderEdge edge2) {
			...
			if (timeUntilGhostChoice == 0) {
				...
				for (int i = 0; JunctionGraph.paths[path1 + i] >= 0
						&& JunctionGraph.paths[path2 + i] == JunctionGraph.paths[path1 + i]; ++i) {
				...
				if (border.edge.containsPowerPill) {
					...
					if (nodeIndex >= 0 && b.containsPowerPill[nodeIndex]) {
						...
						if (pacmanNode.edge == node.edge) {
							...
							int pacmanDistToGhostJunction = node.edge.getDistanceToJunction(pacmanNode,
									border.ghostJunction);
								...
								for (int g = 0; g < border.ghostDist.length; ++g) {
									if ((border.closerGhosts & (1 << g)) != 0) {
										...
										if (border.ghostDist[g] == 0) {
											ghostDistToPowerPill = game
													.getShortestPathDistance(b.ghosts[g].currentNodeIndex, node.index);
										}
										if (statLog) {
											log("ghost " + g + ", ghostDist " + ghostDistToPowerPill + ", pacdist: "
													+ pacmanDistToPowerPill);
										}
							...
							int pacmanDistToPowerPill = border.pacmanDist - distToGhostJunction;// game.getShortestPathDistance(b.pacmanLocation,
																								// nodeIndex);
							if (statLog) {
								log("checkPowerPillsOnBorderEdges, pill " + node + ", pacmanDist: "
										+ pacmanDistToPowerPill);
							}
							boolean inReach = true;
							for (int g = 0; g < border.ghostDist.length; ++g) {
								if ((border.closerGhosts & (1 << g)) != 0 && border.ghostDist[g] + distToGhostJunction
										- EAT_DISTANCE <= pacmanDistToPowerPill) {
									if (statLog) {
										log("closer ghost: " + g);
									}
									inReach = false;
	 ...
	 *
	 * @param arr
	 *            every element contains bit mask for a ghost, bit set means
				...
				if ((arr[i] & (1 << j)) != 0) {
					...
					for (int k = 0; k < arr.length; ++k) {
						if (k == i) {
							...
							arr2[k] = (arr[k] & mask) | ((arr[k] >> 1) & ~mask);
						}
					}
					if (match(arr2, n - 1)) {
						return true;
