...
public class AStarSearchTest {
	...
	public void simpleGridWorldTest() {
		// TODO: It seems problematic that the goal location has to be hard-coded into the heuristic.
		//       Might want to fix this so that this information comes from the problem ... but that
		//       might restrict all problems to being grid-world type problems with a goal location.
		Heuristic<GridAction,GridState> manhattan = new Heuristic<GridAction,GridState>() {
			// Simple Manhattan distance heuristic
			@Override
			public double h(GridState s) {
				return Math.abs(s.x - KNOWN_GOAL_X) + Math.abs(s.y - KNOWN_GOAL_Y);
			}
			
		};
		Search<GridAction,GridState> search = new AStarSearch<>(manhattan);
