...
public class MountainCarVisualizer extends AbstractVisualizer
		implements ValueFunctionDataProvider, AgentOnValueFunctionDataProvider, GlueStateProvider {
	...
	// This is a little interface that will let us dump controls to a panel
	// somewhere.
	...
	public Vector<Double> queryAgentValues(Vector<Observation> theQueryObs) {
		...
		if (theValueResponse == null) {
			if (!printedQueryError) {
				...
				System.err.println(
						"In the Mountain Car Visualizer: Asked an Agent for Values and didn't get back a parseable message.  I'm not printing this again.");
	...
	// This is the one required from RLVizLib, ours has a forcing parameter.
	// Should update the VizLib
