...
public class DeltaButtons {
	...
	public static void main(String[] args) {
		...
		// Adds delta buttons that will be allowed and set the maximum allowed
		// value (optional).
		...
		// For normal buttons (binary) all values other than 0 are interpreted
		// as pushed.
		// For delta buttons values determine a precision/speed.
		//
		// For TURN_LEFT_RIGHT_DELTA and LOOK_UP_DOWN_DELTA value is the angle
		// (in degrees)
		// of which the viewing angle will change.
		//
		// For MOVE_FORWARD_BACKWARD_DELTA, MOVE_LEFT_RIGHT_DELTA,
		// MOVE_UP_DOWN_DELTA (rarely used)
		// value is the speed of movement in a given direction (100 is close to
		// the maximum speed).
		List<int[]> actions = new ArrayList<int[]>();
		actions.add(new int[] { 10, 1, 1, 1 });
		actions.add(new int[] { 2, -3, -2, -1 });

		// If button's absolute value > max button's value then value = max
		// value with original value sign.

		// Delta buttons in spectator modes correspond to mouse movements.
		// Maximum allowed values also apply to spectator modes.
		// game.addGameArgs("+freelook 1"); //Use this to enable looking around
		// with the mouse.
