"""Runs various QD algorithms on the Rastrigin function. Parameters and text below from Example-Sphere.py, but same params and BCs apply to Rastrigin
...
def rastrigin(sol):
    """
    Rastrgin function evaluation and BCs for a batch of solutions.
...
def create_optimizer(algorithm, dim, seed):
    ...
    Args:
        algorithm (str): Name of the algorithm passed into rastrigin_main.
        dim (int): Dimensionality of the rastrigin function.
...
def rastrigin_main(algorithm,
                dim=20,
                itrs=4500,
                outdir="rastrigin_output",
                log_freq=250,
                seed=None):
    """
        ...
        for itr in range(1, itrs + 1):
            ...
            if itr % log_freq == 0 or final_itr:
                ...
                metrics["Archive Coverage"]["y"].append(
                    len(data)) #/ total_cells * 100) # Maxx: this can be added to chart percentage filled instead of actual filled
