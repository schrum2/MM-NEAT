...
public class CustomExecutor {
	...
	public void runExperiment(Controller<MOVE> pacManController, MASController ghostController, int trials, String description, Game game) {
		...
		for (int i = 0; i < trials; ) {
			// Schrum: 6/27/18
			// This try catch is the reason that our NullPointerException in NodeCollection was not
			// actually crashing the code. Presumably, this is here for the sake of the competition,
			// so that if one player's agent crashes, the other player's agent can still get credit
			// for beating it. However, for the sake of evolution, we should not allow evaluation to
