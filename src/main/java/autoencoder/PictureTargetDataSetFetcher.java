 ...
 *
 */
public class PictureTargetDataSetFetcher extends BaseDataFetcher {
     ...
     * @throws IOException
     */
    public PictureTargetDataSetFetcher(boolean binarize) throws IOException {
        this(binarize, true, true, System.currentTimeMillis(), NUM_EXAMPLES);
    }

    public PictureTargetDataSetFetcher(boolean binarize, boolean train, boolean shuffle, long rngSeed, int numExamples) throws IOException {
        if (!mnistExists()) {
    ...
    public PictureTargetDataSetFetcher() throws IOException {
        this(true);
    }
