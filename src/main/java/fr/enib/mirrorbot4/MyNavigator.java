...
public class MyNavigator<PATH_ELEMENT extends ILocated> extends AbstractUT2004PathNavigator<PATH_ELEMENT>{
	...
	protected void initDirectNavigation (Location dest){
		...
		if (log != null && log.isLoggable(Level.FINE)) {
			log.fine ("LoqueNavigator.initDirectNavigation(): initializing direct navigation"
					+ ", distance " + distance);
		}
	...
	protected void initPathNavigation(Location dest, List<PATH_ELEMENT> path){
		// init the navigation
		if (log != null && log.isLoggable(Level.FINE)) {
			log.fine ("LoqueNavigator.initPathNavigation(): initializing path navigation"
					+ ", nodes " + path.size ());
		}
	...
	private boolean switchToNextNode (){    	
		...
		if (navigCurrentNode == null) {
			...
			runner.reset();
			if (log != null && log.isLoggable(Level.FINE)) {
				log.fine ("LoqueNavigator.switchToNextNode(): switch to next location " + navigCurrentLocation
			...
			// switch to next node
			if (log != null && log.isLoggable(Level.FINE)) {
				log.fine ("LoqueNavigator.switchToNextNode(): switch to next node " + navigCurrentNode.getId().getStringId()
						+ ", distance " + localDistance
	...
	private MyNavigator.Stage navigThroughMover (){
		...
		if (navigStage == MyNavigator.Stage.AWAITING_MOVER){
			...
			if (zDistance > memory.getJumpZBoost())	{
				...
				if (log != null && log.isLoggable(Level.FINER)) {
					log.finer ("LoqueNavigator.navigThroughMover("+stage+"): we are UNDER the mover and mover is RIDING UP ... getting back to waiting position" + 
							", zDistance " + zDistance + ", mover.velocity.z " + mover.getVelocity().z + ", mover "
							+ (moverRidingUp ? "riding UP" : (moverRidingDown ? "riding DOWN" : moverStandingStill ? 
			...
			if (log != null && log.isLoggable(Level.FINER)) {
				log.finer ("Navigator.navigThroughMover("+stage+"): mover arrived" + " | zDistance " + zDistance + 
						", hDistance " + hDistance + ", mover " + (moverRidingUp ? "riding UP" : 
							(moverRidingDown ? "riding DOWN" : moverStandingStill ? "standing STILL" : " movement unknown"))
						+ ", node " + navigCurrentNode.getId().getStringId());
			}
			...
			if ( zDistance > 30 ){
				...
				if (log != null && log.isLoggable(Level.FINER)) { 
					log.finer ("LoqueNavigator.navigThroughMover("+stage+"): riding the mover" + " | zDistance " + 
							zDistance + ", hDistance " + hDistance + ", mover " + (moverRidingUp ? "riding UP" : 
								(moverRidingDown ? "riding DOWN" : moverStandingStill ? "standing STILL" : 
										" movement unknown"))+ ", node " + navigCurrentNode.getId().getStringId()); 	
				}
				runner.stopMovement(focus);
				/*
			...
			if (log != null && log.isLoggable(Level.FINER)) { 
				log.finer ("Navigator.navigThroughMover("+stage+"): exiting the mover" + " | zDistance " + zDistance + 
							", hDistance " + hDistance + ", mover " + (moverRidingUp ? "riding UP" : (moverRidingDown 
							? "riding DOWN" : moverStandingStill ? "standing STILL" : " movement unknown"))
							+ ", node " + navigCurrentNode.getId().getStringId());
			}	        
