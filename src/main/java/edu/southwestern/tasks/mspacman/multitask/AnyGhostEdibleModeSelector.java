...
/**
 * A Mode selector which selects modes based on if there are any edible ghosts
 * at all or not
 * 
 * @author Jacob Schrum
 */
public class AnyGhostEdibleModeSelector extends MsPacManModeSelector {
	...
	/**
	 * constructs this selector and also sets several objective parameter class
	 * values
	 */
	...
	/**
	 * returns what mode this will be based on if there are some edible ghosts
	 * or none 0 if there are no edible ghosts 1 if there are edible ghosts
	 * 
	 * @return mode
	 */
	public int mode() {
	 ...
	 * selected parameters
	 * 
	 * @return an int array holding the score for no edible ghosts in the first
	 *         index and the score for some edible ghosts in the second index
	 */
	public int[] associatedFitnessScores() {
		...
		result[SOME_EDIBLE_GHOST] = punishImproperPowerPillEating ? IMPROPER_POWER_PILL_GHOST_COMBO
				: (levelObjective ? GHOST_AND_LEVEL_COMBO : GHOST_SCORE);
		return result;
	}
}
