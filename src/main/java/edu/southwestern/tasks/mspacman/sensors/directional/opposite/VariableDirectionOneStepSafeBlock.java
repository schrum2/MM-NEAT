 ...
 */
package edu.southwestern.tasks.mspacman.sensors.directional.opposite;
	...
	public double getValue(GameFacade gf) {
		...
		if (threats.length > 0) {
			...
			for (int i = 0; i < ghostIndexes.length; i++) {
				...
				if (threatPair.t2.length <= Constants.EAT_DISTANCE + 2 && gf.isGhostIncoming(dir, ghostIndexes[i])) {
					// System.out.println("Ghost is close and incoming, so
					// direction "+dir+" is dangerous");
					return 0;
				}
			}
		}
		...
		for (int i = 0; i < safePointBuffers.length; i++) {
			if (neighbors[i] != -1) {
				...
				safePointBuffers[i] = VariableDirectionCloserToTargetThanThreatGhostBlock
						.pathReachesTargetSafelyWithBuffer(gf, pair.t1, pair.t2, ghostsToCheck);
				if (dir == i && safePointBuffers[i] > 0) { // early exit
					// System.out.println("\tDir safe:"+dir+": because next
					// point is reachable");
					return 1.0; // can reach junction or power pill in desired
								// direction
				}
			}
		}
		...
		// Pair<Integer, int[]> lairPair = gf.getTargetInDir(current, new
		// int[]{lairExitNode}, dir);
		// if (lairPair.t2.length - 1 <= Constants.EAT_DISTANCE) { // -1 for a
		// single step
		// System.out.println("One step brings pacman within eating distance of
		// lair exit");
		// Executor.hold = true;
		// return 0;
		// }
		...
		for (int i = 0; i < safePointBuffers.length; i++) {
			if (i != dir && neighbors[i] != -1 && safePointBuffers[i] > 2) { // Already
																				// checked
																				// dir
				// System.out.println("\tDir safe:"+dir+": because can backtrack
				// in dir " +i);
				// One step by the ghost and another step by pacman shift the
				// buffer by 2
