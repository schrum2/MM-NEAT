...
public class SplitSpecificGhostBlock extends SpecificGhostBlock {
	...
	public int distances(double[] inputs, int in, GameFacade gf, int lairTime, int pacmanNearestPowerPill,
		...
		if (lairTime == 0) {
			...
			inputs[in++] = !edible ? 1
					: Math.min(pacmanGhostDistance, GameFacade.MAX_DISTANCE) / GameFacade.MAX_DISTANCE;
			inputs[in++] = !edible ? 1
					: Math.min(gf.getShortestPathDistance(lairExit, ghostNode), GameFacade.MAX_DISTANCE)
							/ GameFacade.MAX_DISTANCE;
			inputs[in++] = !edible ? 1
					: pacmanNearestPowerPill == -1 ? 1
							: Math.min(gf.getShortestPathDistance(pacmanNearestPowerPill, ghostNode),
									GameFacade.MAX_DISTANCE) / GameFacade.MAX_DISTANCE;
