...
public class SpecificGhostDistancesBlock extends MsPacManSensorBlock {
	 ...
	 *
	 * @param mask
	 *            which ghosts to include (include if index is true)
	 * @param threats
	 *            whether sensing threats or other
	 */
	...
	public int incorporateSensors(double[] inputs, int in, GameFacade gf, int lastDirection) {
		...
		for (int i = 0; i < CommonConstants.numActiveGhosts; i++) {
			if (mask[i]) {
				...
				// double distance = present ?
				// gf.getShortestPathDistance(current,
				// gf.getGhostCurrentNodeIndex(i)) : GameFacade.MAX_DISTANCE;
				// Try using ghost paths instead
				double distance = present ? gf.getGhostPathDistance(i, current) : GameFacade.MAX_DISTANCE;
				assert gf.getGhostPath(i, current).length == gf.getGhostPathDistance(i,
						current) : "Different methods of calculating ghost path length should be the same";
				// inputs[in++] = Util.scaleAndInvert(distance,
