...
/**
 * 
 * @author rollinsa The predator fitness function appearing in Constructing
 ...
 * 
 *         Predator fitness score: 25 ------ if both prey caught (20c / n) +
 *         (2c) + (20d / n) ------ otherwise c = number of prey caught n = total
 *         number of prey d = normalized sum of distances from the predator to
 *         each prey at the end of the simulation
 * 
 *         Encourages catching all prey with an emphasis on being as close as
 *         possible in distance to catching those that weren't caught
 */
public class PredatorRawalRajagopalanMiikkulainenObjective<T extends Network> extends GridTorusObjective<T> {
	...
	public PredatorRawalRajagopalanMiikkulainenObjective() {
		// because of the equation for the fitness function given by the RRM
		// paper, there must be two prey
	...
	public double fitness(Organism<T> individual) {
		...
		// ------ (20c / n) + (2c) + (20d / n) ------ (generalized for variable
		// number of prey/preds)
		// For general equation (not requirement of 2 prey) would be: Score =
		// %PreyCaught + MinimizedDistance
