 ...
 * @author Jacob Schrum
 */
public class OpponentAndTeammateRelativeMovementOutputModel implements UT2004OutputInterpretation {
	...
	public BotAction interpretOutputs(@SuppressWarnings("rawtypes") UT2004BotModuleController bot, double[] outputs) {
		...
		Player friend = bot.getPlayers().getNearestFriend(MEMORY_TIME);
		...
		if(opponent != null) {
			...
			assert friend != null : "This module should only be used if an agent is visible. Since the opponent is null, there should be a friend: " + friend;
			shoot = false; // Do not shoot friends
			return new OpponentRelativeMovementAction(friend, moveForwardBackward, strafeLeftRight, shoot, jump);
		}
	}
	...
	public UT2004OutputInterpretation copy() {
		return new OpponentAndTeammateRelativeMovementOutputModel();
	}
}
