...
public class NNSimpleEvaluationFunction<T extends Network> extends NNEvaluationFunction<T> {
	...
	private static final int RESOURCE_PLAYER = -1; // "player" value for resource "unit" is -1
	 ...
	 * @author alicequint
	 */
	public double[] gameStateToArray(GameState gs, int playerToEvaluate){
		...
		for(int i = 0; i < pgs.getWidth(); i++){
			for(int j = 0; j < pgs.getHeight(); j++){
				...
				if(currentUnit != null && currentUnit.getPlayer() != RESOURCE_PLAYER && currentUnit.getType().name.equals("Base")){
					playerAdjustment = (currentUnit.getPlayer() == playerToEvaluate) ? 0 : ENEMY_ADJUSTMENT;
					...
					if(currentUnit.getPlayer() != playerToEvaluate){
						//record location so it can be used later
						// TODO: Why is this calculated in this way?
			...
			for(int j = 0; j < pgs.getHeight(); j++){
				...
				if(currentUnit != null){
					...
					case "Light": 
						...
						if(currentUnit.getPlayer() == playerToEvaluate && enemyBaseLocation != -1){
							currentDistance = distance(currentUnit, enemyBaseLocation);
							//incremental calculation of the avg.
		...
		for(int i = 0; i <= BARRACKS; i++){
			inputs[i] /= (pgs.getWidth()*pgs.getHeight()/2.0);
			inputs[i+ENEMY_ADJUSTMENT] = (pgs.getWidth()*pgs.getHeight()/2.0);
		}

		// TODO: Calculation does not match comment
		...
		// TODO: Fix magic numbers 6 and 3
	...
	private double distance(Unit currentUnit, int enemyBaseLocation){
		...
		// TODO: This calculation exists in a utility class somewhere
		//distance: sq. root of ((x2-x1)^2 + (y2-y1)^2)
		return Math.sqrt((currentUnit.getX() - baseX)*(currentUnit.getX() - baseX) + (currentUnit.getY() - baseY)*(currentUnit.getY() - baseY));
	}
