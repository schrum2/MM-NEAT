...
/**
 * D4W1 convolutional architecture where the connection from the input to the first hidden layer is 3x3,
 * the connection from the first hidden layer to the second hidden layer is 1x1, the connection from 
 * the second hidden layer to the third hidden layer is 3x3, and the connection from the third hidden to
 * the fourth hidden layer is 1x1
 * @author Devon Fulcher
 */
public class TuneUp3x31x13x31x1 implements SubstrateArchitectureDefinition{
	...
	public List<SubstrateConnectivity> getSubstrateConnectivity(HyperNEATTask hnt) {
		...
		FlexibleSubstrateArchitecture.connectHiddenToHidden(substrateConnectivity, networkHiddenArchitecture, 1, 1, 2, 3);
