 ...
 * predators to most distance predators
 * 
 * @author rollinsa
 *
 */
public class TorusPredatorsByProximitySensorBlockTest {
	...
	public void testSensorValues() {
		...
		// move the predators to locations 11,22 , 48,85 and 99, 36 respectively
		// and prey[1] to location 50,57
		preds[0].move(11, 22);
		preds[1].move(48, 85);
		preds[2].move(99, 36);
		prey[1].move(50, 57);

		// from prey[0] at point 0,0 , the closest predators are pred[0] then
		// pred[2], then pred[1]
		assertEquals(Arrays.toString(block.sensorValues(prey[0], world, preds, prey)),
				"[0.11, 0.22, -0.01, 0.36, 0.48, -0.15]");
		// from prey[1] at point 50,57 , the closest predators are pred[1] then
		// pred[2], then pred[0]
		assertEquals(Arrays.toString(block.sensorValues(prey[1], world, preds, prey)),
				"[-0.02, 0.28, 0.49, -0.21, -0.39, -0.35]");
