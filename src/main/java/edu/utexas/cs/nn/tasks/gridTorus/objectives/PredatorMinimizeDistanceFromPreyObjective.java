 ...
 * @author rollinsa
 * 
 *         reward the predators for being as close to the prey as possible at
 *         the end of the game if the prey are dead then distance from that prey
...
public class PredatorMinimizeDistanceFromPreyObjective<T extends Network> extends GridTorusObjective<T> {
	...
	/**
	 * Finds the sum of the sum of the distances from each predator to each
	 * prey. In other words, for each predator, get the distances to each prey
	 * in a list (array). Then, add up these distances so that there is a list
	 * (array) of sums of distances for each predator. Then add up these sums of
	 * distances for each predator to have one total sum.
	 * 
	 * @param preds
	 * @param prey
	...
	@Override
	/**
	 * reward the predators for being as close to the prey as possible at the
	 * end if the prey are dead then distance from that prey is counted as zero
	 * (meaning that the predators are also inherently encouraged to eat the
	 * prey)
	 */
	public double fitness(Organism<T> individual) {
		TorusAgent[] preds = game.getPredators();
		TorusAgent[] prey = game.getPrey();
		// want this to be as low as possible to minimize distance to prey for
		// each pred
	 ...
	 * distance from each the prey for each pred
	 */
