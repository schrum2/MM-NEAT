...
public class TWEANNModulesNicheDefinition extends NicheDefinition<TWEANN> {
	 ...
	 *
	 * @param modes
	 *            number modes of candidate network
	...
	// // Look through niches with more modes as well (PROBLEM WITH GAPS! ie 3
	// modes but not 2 modes)
	...
	// if(Math.abs(nicheScores.get(i) - bestOverallScores.get(i)) <
	// Double.MIN_VALUE){
	// // Then more modes are justified
	// //System.out.println("MM with " + modes + " modes because of niche " +
	// nicheModes);
	// return true;
	// }
	// }
	...
	// HashMap keys are mode numbers, vector indices are objective indices,
	// doubles are best scores
	...
	private void updateBestNicheScores(int numModes, Score<TWEANN> individual) {
		int numScores = individual.numObjectives() - 1; // Ignore diversity
														// objective
