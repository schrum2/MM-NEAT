	...
	public Breve2DAction getAction(Breve2DGame game) {
		...
		Collections.sort(monsters, new Comparator<Agent>() {
			...
			public int compare(Agent o1, Agent o2) {
				double a1 = CartesianGeometricUtilities.signedAngleFromSourceHeadingToTarget(player, o1.getPosition(),
						playerHeading);
				double a2 = CartesianGeometricUtilities.signedAngleFromSourceHeadingToTarget(player, o2.getPosition(),
						playerHeading);
		...
		for (int i = 0; i < monsters.size(); i++) {
			if (!monsters.get(i).isDead()) {
				...
				if (!CartesianGeometricUtilities.sourceHeadingTowardsTarget(playerHeading, player, loc,
						3 * Math.PI / 4)) {
		...
		if (biggestDiff > 0) {
			...
			Tuple2D closer = leftDistance < rightDistance ? monsters.get(left).getPosition()
					: monsters.get(right).getPosition();
			double borderAngle = loc.angleBetweenTargets(closer, player);
			boolean tooSharp = borderAngle < Math.PI / 4;

			// double anglePreviousAndMidpoint = previousTarget == null ? 0 :
			// player.angleBetweenTargets(loc, previousTarget);
			...
			double angleTo = CartesianGeometricUtilities.signedAngleFromSourceHeadingToTarget(player, loc,
					CartesianGeometricUtilities.restrictRadians(playerHeading + Math.PI));
		...
		double angle = CartesianGeometricUtilities.signedAngleFromSourceHeadingToTarget(player,
				nearestMonster.getPosition(), playerHeading);
		if (angle >= Math.PI / 2) {
			// turn = angle / Math.PI;
