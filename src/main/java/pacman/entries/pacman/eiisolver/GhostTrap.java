...
public class GhostTrap {
	...
	public static MOVE rigTrap(Game game, Board b) {
		...
		if (ghostDist >= 0 && ghostDist < 40) {
			...
			if (lastPath != null) {
				...
				MOVE bestMove = MOVE.NEUTRAL; // wait until nearest ghost gets
												// very close
	...
	/*
	 * public static MOVE rigTrapOld(Game game, Board b) { int ghostDist =
	 * Search.distToFarthestGhostInTrain(); if (ghostDist >= 0 && ghostDist <
	 * 40) { System.out.println("In ghost train!"); Log.println(
	 * "In ghost train!"); if (lastPath != null) { //return
	 * lastPath.getMove(game, b); } // walk to closest trap PathToPill bestPath
	 * = null; int shortestDist = 1000; int distToClosestGhost = 0; for
	 * (PathToPill path : traps) { if
	 * (b.containsPowerPill[path.powerPill.index]) { int dist =
	 * game.getShortestPathDistance(b.pacmanLocation, path.path[0].index); int
	 * gDist = ghostDist(game, b, path.path[0].index); if (dist < gDist + 2) {
	 * System.out.println("To " + path.path[0] + ": pacmanDist " + dist +
	 * ", gDist: " + gDist); if (path.isOnPath(b.graph.nodes[b.pacmanLocation]))
	 * { bestPath = path; shortestDist = 0; } else { if (dist < shortestDist) {
	 * System.out.println("Better dist"); shortestDist = dist; bestPath = path;
	 * distToClosestGhost = gDist - dist; } } } } } lastPath = bestPath; if
	 * (lastPath != null) { System.out.println("Set up trap towards " +
	 * lastPath.powerPill); Log.println("Set up trap towards " +
	 * lastPath.powerPill); lastPath = bestPath; MOVE bestMove = MOVE.NEUTRAL;
	 * if (distToClosestGhost <= 3) { bestMove = lastPath.getMove(game, b); }
	 * return bestMove; } } lastPath = null; return null; }
	 */
	...
	private static void constructLevel2(Game game, Board b) {
		...
		PathToPill p4 = constructPath(game, b, 92, 104, new int[] { 92, 72, 92, 84, 104, 84, 104, 104 }, 103, 104,
				MOVE.RIGHT);
		traps.add(p1);
		traps.add(p2);
		traps.add(p3);
	...
	private static PathToPill constructPath(Game game, Board b, int pillY, int pillX, int[] edgeCoords, int lastY,
			int lastX, MOVE firstMoveIntoPath) {
		...
		public MOVE getMove(Game game, Board b) {
			...
			for (int i = 0; i < path.length - 1; ++i) {
				if (path[i] == pacmanNode) {
					// pacman is already on the trapping path; let her go to the
					// next step
