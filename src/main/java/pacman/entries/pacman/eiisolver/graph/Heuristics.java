...
public class Heuristics {
	...
	private void setNodeScore() {
		...
		/*
		 * if (!Search.pacmanEvaluation && weakOpponent) { for (int i = 0; i <
		 * graph.nodes.length; ++i) { Node n = graph.nodes[i]; if (n.y <= 54) {
		 * int pacmanX = graph.nodes[Search.b.pacmanLocation].x; if ((pacmanX <=
		 * 54) == (n.x <= 30)) { nodeScore[i] = 15000; } } } }
		 */
	}
	...
	private void setPowerPillScore() {
		...
		System.out.println("power pill score: " + powerPillScore + ", exist: " + existNonKilling + ", time: "
				+ game.getCurrentLevelTime() + ", powerpillsOnBoard: " + b.nrPowerPillsOnBoard);
	}
