...
//		boolean wasUp = false;
		...
		for(int level = 1;level<numberOfChunks;level++) {
			...
			if(right) {
				...
				for(int x = (int) previousMove.getX()+16;x<(int) previousMove.getX()+32;x++) {
					for(int y = (int) previousMove.getY();y<(int) previousMove.getY()+14;y++) {
//						System.out.println(x+", "+y);
//						System.out.println(previousMove.getX()+", "+previousMove.getY());

						oneLevel.get(y).set(x, levelInList.get(level).get((int) (y - previousMove.getY())).get((int) (x-16-previousMove.getX())));
					}
				}
				//wasRight = true;
				d = Direction.RIGHT;
				previousMove=new Point((int) previousMove.getX()+16,(int) previousMove.getY());
				...
				if(d.equals(Direction.UP)) { //add null lines all on top
					...
					for(int y = (int) previousMove.getY();y<previousMove.getY()+14;y++) {
						for(int x = (int) previousMove.getX();x<previousMove.getX()+16;x++) {
					...
					for(int y = (int) previousMove.getY()+14;y<previousMove.getY()+28;y++) {
						for(int x = (int) previousMove.getX();x<previousMove.getX()+16;x++) {
							oneLevel.get(y).set(x, levelInList.get(level).get((int) (y -14- previousMove.getY())).get((int) (x-previousMove.getX())));
						}
					}
					previousMove=new Point((int) previousMove.getX(),(int) previousMove.getY()+14);
