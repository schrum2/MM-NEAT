...
C
C FILE 5 CONTAINS THE MOLECULES GENERATED BY FIRST PART
C FILE 6 CONTAINS THE RESULTS FOR EACH OF THESE MOLECULES
C
      BVAL1=336.51
      BVAL2=147.67
C      READ(5,*) ITER,TEMP
...
C
C      READ(5,*) MAX,CVAL,BEST
C      WRITE(6,*) MAX,CVAL,BEST
      READ(5,*) MAX
C      WRITE(6,*) MAX
...
C
      ...
      IF(VAL1.GT.0) THEN
        VAL=SQRT((VAL1-BVAL1)**2+(VAL2-BVAL2)**2)
C        WRITE(6,*) "VAL = ",VAL,VAL1,VAL2
      ELSE
        WRITE(6,*) "PROBLEM FOUND",VAL
      ENDIF
      RETURN
      END
C
