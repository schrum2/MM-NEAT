 ...
 * sensors that focus specifically on the 4th ghost, which in the Legacy team is
...
public class RandomFocusCheckEachDirectionMediator extends AlternateCheckEachDirectionMediator {
	...
	public RandomFocusCheckEachDirectionMediator() {
		...
		// Distances
		blocks.add(
				new VariableDirectionSpecificEdibleGhostDistanceBlock(irrelevantStartingDirection, randomGhostIndex));
		blocks.add(
				new VariableDirectionSpecificThreatGhostDistanceBlock(irrelevantStartingDirection, randomGhostIndex));
		if (incoming) {
			blocks.add(new VariableDirectionSpecificApproachingEdibleGhostDistanceBlock(irrelevantStartingDirection,
					randomGhostIndex));
			blocks.add(new VariableDirectionSpecificApproachingThreatGhostDistanceBlock(irrelevantStartingDirection,
					randomGhostIndex));
		}
		// Obstructions
		blocks.add(new VariableDirectionCloserToJunctionThanThreatGhostBlock(irrelevantStartingDirection,
				new int[] { randomGhostIndex }));
		blocks.add(new VariableDirectionCloserToDepthTwoJunctionThanThreatGhostBlock(irrelevantStartingDirection,
				new int[] { randomGhostIndex }));
		blocks.add(new VariableDirectionCloserToPowerPillThanThreatGhostBlock(irrelevantStartingDirection,
				new int[] { randomGhostIndex }));
		blocks.add(new VariableDirectionCloserToEdibleGhostThanThreatGhostBlock(irrelevantStartingDirection,
				new int[] { randomGhostIndex })); // Does not account for edible
													// ghost movement
		...
		if (incoming) {
			blocks.add(
					new VariableDirectionKStepIncomingEdibleGhostCountBlock(irrelevantStartingDirection, mask, true));
			blocks.add(
					new VariableDirectionKStepIncomingThreatGhostCountBlock(irrelevantStartingDirection, true, mask));
			blocks.add(
					new VariableDirectionKStepIncomingThreatGhostCountBlock(irrelevantStartingDirection, false, mask));
		}
		// Misc
