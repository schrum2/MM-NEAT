...
public class SpecificGhostBlock extends MsPacManSensorBlock {
	...
	public int incorporateSensors(double[] inputs, int in, GameFacade gf, int lastDirection) {
		...
		final int pacmanNearestPowerPill = gf.getNumActivePowerPills() == 0 ? -1
				: gf.getClosestNodeIndexFromNodeIndex(pacmanNode, gf.getActivePowerPillsIndices());
		final int ghostNode = proximityOrder ? gf.ghostLocationByProximity(ghostIndex)
				: gf.getGhostCurrentNodeIndex(ghostIndex);
		// edible time
	...
	public int distances(double[] inputs, int in, GameFacade gf, int lairTime, int pacmanNearestPowerPill,
			int pacmanNode, int ghostNode, int lairExit) {
		if (lairTime == 0) {
			...
			inputs[in++] = Math.min(gf.getShortestPathDistance(lairExit, ghostNode), GameFacade.MAX_DISTANCE)
					/ GameFacade.MAX_DISTANCE;
			inputs[in++] = pacmanNearestPowerPill == -1 ? 1
					: Math.min(gf.getShortestPathDistance(pacmanNearestPowerPill, ghostNode), GameFacade.MAX_DISTANCE)
							/ GameFacade.MAX_DISTANCE;
	...
	public int directions(double[] inputs, int in, int referenceDir, int[] neighbors, int lairTime, int targetDir,
			boolean edible) {
