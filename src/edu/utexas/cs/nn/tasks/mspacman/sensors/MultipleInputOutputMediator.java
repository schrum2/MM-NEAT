...
public class MultipleInputOutputMediator extends MsPacManControllerInputOutputMediator {
    ...
    /**
     * constructor to obtain the list of mediators in order to later combine them into one
     * @param mediators, array of MsPacManControllerInputOutputMediator's
     */
    ...
    @Override
    /**
     * clear and reset each MsPacManControllerInputOutputMediator the array of mediators
     */
    public void reset() {
        for (int i = 0; i < mediators.length; i++) {
            mediators[i].reset();
    ...
    @Override
    /**
     * retrieves and returns the sensor values based on all of the different mediators provided/requested
     * (combined into one)
     * 
    ...
    @Override
    /**
     * find and return the number of inputs based on the number of inputs in each mediator (finds the sum)
