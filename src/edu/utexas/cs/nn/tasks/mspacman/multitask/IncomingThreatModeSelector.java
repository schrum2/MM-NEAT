...
/**
 * A Mode selector which selects between 2 modes based on the following: 0) The
 * threats are all incoming 1) The threats are not all incoming (so there is an
 * escape available)
 * 
 * @author Jacob Schrum
 */
public class IncomingThreatModeSelector extends MsPacManModeSelector {
	 ...
	 * The threats are all incoming 1) The threats are not all incoming (so
	 * there is an escape available)
	 * 
	 * @return mode
	 */
	public int mode() {
	 ...
	 * 
	 * @return an int array holding the score for if all threats are incoming in
	 *         the first index and the score for if not all threats are incoming
	 *         (if escape is available) in the second index
	 */
