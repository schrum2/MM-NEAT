 ...
 * "scentMaps" is dangerous or safe. An area (based on pacman's current
 * location) is determined to be dangerous if pacman has died more in that area
 * than the average area, and is safe otherwise
 * 
 * @author Jacob Schrum
 */
public class DangerousAreaModeSelector extends MsPacManModeSelector {
	...
	/**
	 * updates the scentMaps based on the deathCount of pacman. Also finds the
	 * mazeAverages for the death counts of pacman in the areas of the scentMaps
	 */
		...
		if (scentMaps != null) {
			...
			for (int i = 0; i < mazeAverages.length; i++) {
				if (scentMaps.containsKey(i)) { // maze present
					HashMap<Integer, Integer> mazeDeaths = scentMaps.get(i); // (loc,count)
																				// pairs
					double[] counts = ArrayUtil
							.doubleArrayFromList(Collections.list(Collections.enumeration(mazeDeaths.values())));
	 ...
	 *
	 * @return whether or not the area is dangerous or safe (0 is dangerous, 1
	 *         is safe)
	 */
	public int mode() {
		...
		if (scentMaps.containsKey(maze)) {
			...
			if (mazeDeaths.containsKey(current)) {
				...
				if (count / n > mazeAverages[maze]) {
					// System.out.println(count +" > " +mazeAverages[maze] + "
					// deaths");
					return DANGEROUS;
				}
			}
	...
	@Override
	/**
	 * gets the associated fitness scores with this mode selector based on the
	 * danger of this area
	 * 
	 * @return an int array holding the score for if the area is dangerous in
	 *         the first index and the score for if the area is safe in the
	 *         second index
	 */
