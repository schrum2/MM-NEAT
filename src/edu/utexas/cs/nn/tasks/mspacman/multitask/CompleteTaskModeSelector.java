...
public class CompleteTaskModeSelector extends MsPacManModeSelector {
    ...
    /**
     * constructs this selector and sets the definition of "closeGhostDistance" based on command line parameters
     */
    ...
    /**
     * sets the game mode based on if the ghosts are edible, no ghosts are edible and a ghost is near, 
     * or no ghosts are edible and there are no ghosts near
     * 0 if the ghosts are edible
     * 1 if the ghosts are not edible and there is one near
     * 2 if the ghosts are not edible and the ghosts are not near
     * @return mode
     */
    public int mode() {
        // Eat edible ghosts
    ...
    @Override
    /**
     * gets the associated fitness scores with this mode selector based on human defined close ghost distance
     * @return an int array holding the score for if the ghosts are edible in the first index, the score
     * for if there are no edible ghosts and a ghost is near in the second index, and the score for if
     * there are no edible ghosts and the ghosts are not near in the third index
     */
