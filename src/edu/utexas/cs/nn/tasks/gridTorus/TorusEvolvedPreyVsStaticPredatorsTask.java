...
public class TorusEvolvedPreyVsStaticPredatorsTask<T extends Network> extends NoisyLonerTask<T> implements TUGTask, NetworkTask {
	 ...
	 * one or more prey is evolved while predators are static
	...
	public Pair<double[], double[]> oneEval(Genotype<T> individual, int num) {
		...
		TorusPredPreyController[] evolvedAgents = new TorusPredPreyController[numPreys];    	
		for(int i = 0; i < numPreys; i++){
			//false to indicate that this is not a predator
			NNTorusPredPreyAgent<T> preyAgent = new NNTorusPredPreyAgent<T>(individual, false);
			evolvedAgents[i] = preyAgent.getController(); 
		}
		exec = new TorusWorldExec(); 
		TorusPredPreyGame game; 
		if (CommonConstants.watch) { 
			game = exec.runGameTimed(staticAgents, evolvedAgents, true); 
		} else {
			game = exec.runExperiment(staticAgents, evolvedAgents);
		}
