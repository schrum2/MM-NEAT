...
public class PredatorMinimizeDistanceFromPreyObjectiveTest {
	...
	public void setUp() throws Exception {
		Parameters.initializeParameterCollections(new String[]{"io:false","netio:false", "torusTimeLimit:1000", 
				...
				"allowDoNothingActionForPredators:true", "allowDoNothingActionForPreys:true", "torusPreys:2", "torusPredators:3",
				"staticPredatorController:edu.utexas.cs.nn.gridTorus.controllers.AggressivePredatorController", "torusSenseTeammates:true"});
		MMNEAT.loadClasses();
	}
	...
	public void test() {
		...
		//each predator is 0 distance away from each prey, meaning the distance double array should be the following:
		// [ {0,0} , {0,0} , {0,0} ] 
		// the sum array should hold [0,0,0], so the final value should be 0, which is the best possible score
		...
		preds[1].move(50,50);
		preds[2].move(50,50);
		//each predator is 100 distance away from each prey, meaning the distance double array should be the following:
		// [ {100,100} , {100,100} , {100,100} ] 
		// the sum array should hold [200,200,200], so the final value should be -600, which is the worst possible score
		fitness = objective.score(game, null);
		...
		preds[0].move(-50, -50);
		// [ {0,0} , {100,100} , {100,100} ] 
		// the sum array should hold [0,200,200], so the final value should be -400
		fitness = objective.score(game, null);
		...
		preds[1].move(-18, 11);
		// [ {0,0} , {71,71} , {100,100} ] 
		// the sum array should hold [0,142,200], so the final value should be -342
		fitness = objective.score(game, null);
		...
		preds[2].move(41, -44);
		// [ {0,0} , {71,71} , {15,15} ] 
		// the sum array should hold [0,142,30], so the final value should be -172
		fitness = objective.score(game, null);
		...
		prey[1].move(20, 20);
		// [ {0,40} , {71,53} , {15,43} ] 
		// the sum array should hold [40,124,58], so the final value should be -222
		fitness = objective.score(game, null);
		...
		// [ {40,0} , {71,0} , {15,0} ] 
		// the sum array should hold [40,71,15], so the final value should be -126
		fitness = objective.score(game, null);
