...
public class PreyMaximizeDistanceFromPredatorsObjective<T extends Network> extends GridTorusObjective<T> {
	...
	/**
	 * Finds the sum of the sum of the distances from each prey to each
	 * predator. In other words, for each prey, get the distances to each
	 * predator in a list (array). Then, add up these distances so that there is
	 * a list (array) of sums of distances for each prey. Then add up these sums
	 * of distances for each prey to have one total sum.
	 * 
	 * @param preds
	 * @param prey
