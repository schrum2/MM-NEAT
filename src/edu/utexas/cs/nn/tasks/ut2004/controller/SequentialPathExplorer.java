...
public abstract class SequentialPathExplorer implements BotController {
    ...
    private BotAction handleNavPointNavigation(UT2004BotModuleController bot) {
        if (bot.getNavigation().isNavigating()) {
            ...
            return new EmptyAction("[Still going to " + targetNavPoint.getId().getStringId() + "]" + (targetNavPoint.isInvSpot() ? targetNavPoint.getItemClass().getName() : ""));
        }
    ...
    public void initialize(UT2004BotModuleController bot) {
        ...
        autoFixer = new UT2004PathAutoFixer(bot.getBot(), (UT2004PathExecutor<ILocated>) bot.getPathExecutor(), bot.getFwMap(), bot.getNavBuilder());
