...
/**
 * Wrapper for Agent's collecting various statistical information. Eg.: <ul>
 * <li>Damage caused</li> <li>Players killed</li> <li>Damage suffered</li>
 * <li>Number of times the bot hit a wall</li> <li>TODO add more</li> </ul>
 *
 * @author Ik
 */
public class GameDataCollector implements Serializable {
    ...
    protected int damageEvents = 0; // Number of times bot is responsible for PlayerDamaged message
    ...
    public void registerListeners(final UT2004BotModuleController bot) {
        ...
        bot.getWorld().addObjectListener(Player.class, WorldObjectDisappearedEvent.class, disappearPlayerListener = new IWorldObjectEventListener<Player, WorldObjectDisappearedEvent<Player>>() {
            @Override
            public void notify(WorldObjectDisappearedEvent<Player> dp) {
                ...
                if (dis == null || lastPlayerKilled == null || !lastPlayerKilled.equals(dis.getId()) || diff > 15) {
                    //System.out.println("Enemy escaped: " + dis.getName());
                    enemyEscapes++;
                }
            }
        });
        ...
        bot.getWorld().addObjectListener(Player.class, WorldObjectUpdatedEvent.class, seePlayerListener = new IWorldObjectEventListener<Player, WorldObjectUpdatedEvent<Player>>() {
            @Override
            public void notify(WorldObjectUpdatedEvent<Player> sp) {
                ...
                opponentSightings++;
                // The more the bot looks at the player, the smaller this will be.
                // Goal is to minimize the angle, corresponds to maximizing focus.
        ...
        bot.getWorld().addEventListener(PlayerKilled.class, killListener = new IWorldEventListener<PlayerKilled>() {
            ...
            public void notify(PlayerKilled killed) {
                ...
                if (killed.getKiller().equals(info.getId())) {
                    ...
                    frags++;
                    currentStreak++;
                    //TODO: Carefull, if someone else kills him, it won't get reset. Problem?
        ...
        bot.getWorld().addEventListener(PlayerDamaged.class, damageListener = new IWorldEventListener<PlayerDamaged>() {
            ...
            public void notify(PlayerDamaged damaged) {
                ...
                if (!damaged.getId().equals(info.getId())) {
                    ...
                    int previousWeaponDamage = weaponDamage.get(damaged.getWeaponName()) == null ? 0 : weaponDamage.get(damaged.getWeaponName());
                    weaponDamage.put(damaged.getWeaponName(), previousWeaponDamage + damaged.getDamage());
                    int previousDamage = damageDone.get(damaged.getId().getStringId()) == null ? 0 : damageDone.get(damaged.getId().getStringId());
        ...
        bot.getWorld().addEventListener(BotDamaged.class, botDamageListener = new IWorldEventListener<BotDamaged>() {
            ...
            public void notify(BotDamaged damaged) {
                //System.out.println("Bot damaged: " + agent.agentmemory.game.getTime() + ":" + damaged);
                damageSuffered += damaged.getDamage();
            }
        });

        // wall hits
        bot.getWorld().addEventListener(WallCollision.class, wallCollisionListener = new IWorldEventListener<WallCollision>() {
            @Override
        ...
        // Self info
        bot.getWorld().addObjectListener(Self.class, WorldObjectUpdatedEvent.class, selfListener = new IWorldObjectEventListener<Self, WorldObjectUpdatedEvent<Self>>() {
            @Override
