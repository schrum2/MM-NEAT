...
public class ControllerBot extends UT2004BotModuleController {
     ...
     * to control it
     *
     * @param name In-game name of bot
     * @param controller the brain/network controlling bot
     * @param host host of pre-loaded server
     * @param botPort port on server to connect bot
     * @throws PogamutException
     */
    public static GameDataCollector[] launchBot(IUT2004Server server, String name, BotController[] controllers, int evalSeconds, int desiredSkill, String host, int botPort) {
        ...
        for (int i = 0; i < controllers.length; i++) {
            ...
            params[i] = new ControllerBotParameters(server, controllers[i], name, collectors[i], evalSeconds, desiredSkill, botPort);
        }
        try {
    ...
    public static void main(String[] args) {
        ...
        ControllerBotParameters b1 = new ControllerBotParameters(null, new RandomNavPointPathExplorer(), "ExplorationBot", new GameDataCollector(), 500, 1, port);
        ControllerBotParameters b2 = new ControllerBotParameters(null, new DummyController(), "DummyBot", new GameDataCollector(), 500, 1, port);
        UT2004BotDescriptor test = new UT2004BotDescriptor().setController(ControllerBot.class).setAgentParameters(new IRemoteAgentParameters[]{b1, b2});
        multi.setMain(true).startAgents(test);
//        GameDataCollector data = launchBot(null, "DummyBot", new DummyController(), 500, 1, "localhost", port);
