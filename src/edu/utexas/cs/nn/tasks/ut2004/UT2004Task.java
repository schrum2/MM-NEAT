...
public abstract class UT2004Task<T extends Network> extends NoisyLonerTask<T> implements NetworkTask {
    ...
    public Pair<double[], double[]> oneEval(Genotype<T> individual, int num) {
        ...
        //config.setOptions(botprizeMod + "?timelimit=" + evalMinutes + "?fraglimit=0?GoalScore=0?DoUplink=False?UplinkToGamespy=False?SendStats=False?bAllowPrivateChat=False?bAllowTaunts=False?bEnableVoiceChat=False?bAllowLocalBroadcast=False?BotServerPort=" + botPort + "?ControlServerPort=" + controlPort + "?ObservingServerPort=" + observePort);
        config.setOptions(botprizeMod + "?fraglimit=0?GoalScore=0?DoUplink=False?UplinkToGamespy=False?SendStats=False?bAllowPrivateChat=False?bAllowTaunts=False?bEnableVoiceChat=False?bAllowLocalBroadcast=False?BotServerPort=" + botPort + "?ControlServerPort=" + controlPort + "?ObservingServerPort=" + observePort);
        ...
        while (result == null) {
            ...
            try {
                ...
                while (server.getAgents().size() > 0 || server.getNativeAgents().size() > 0 || server.getPlayers().size() > 0) {
                ...
                try {
                    ...
                    GameDataCollector[] collectors = ControllerBot.launchBot(server, "EvolvingBot" + gamePort, allBots, evalMinutes * 60, desiredSkill, "localhost", botPort);
                    GameDataCollector stats = collectors[0]; // For now, assume we always want just the first collector
