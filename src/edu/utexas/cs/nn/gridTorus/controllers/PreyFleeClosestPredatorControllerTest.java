...
public class PreyFleeClosestPredatorControllerTest {
	...
	public void testGetAction() {
		...
		assertTrue((prey[0].getX() == 4 && prey[0].getY() == 5) || (prey[0].getX() == 5 && prey[0].getY() == 6) || (prey[0].getX() == 6 && prey[0].getY() == 5));
		
		//reset the prey to be at location 0,0 then move to location 6,11
		prey[0].move((int) -prey[0].getX(), (int) -prey[0].getY());
		prey[0].move(6,11);
		//pred[2] now closest at 5,10, so two possible (and random) movements should be 1,0 or 0,1 and never the other two
