...
public class AggressivePredatorControllerTest {
	...
	public void test() {
		...
		//move a prey to location 10,10 and a predator to location 8,10 and second prey to 50,50 to get it out of the way
		prey[0].move(10,10);
		prey[1].move(50,50);
		preds[0].move(8,10);
		...
		//pred[1] should do either 1,0 or 0,1 to move right or down to approach the prey
		...
		//next action should be to move right (1,0) again from point 9,10 to 10,10 where the prey is
		...
		//pred[2] should do either -1,0 or 0,-1 to move left or up to approach prey[1]
		...
		assertTrue((preds[2].getX() == 50 && preds[2].getY() == 51) || (preds[2].getX() == 51 && preds[2].getY() == 50));
		
		//do one more movement based on the next action
		predAct2 = controller.getAction(preds[2], world, preds, prey);
		preds[2].move(predAct2[0], predAct2[1]);
		//whether at 50,51 or 51,50 before, the predator should now be at 50,50 with the prey
