...
/**
 * Sets crossExcessRate = 1.0 so offspring inherit all innovation numbers from both parents
 * Essentially assures maximum crossover occurs.
 */
	@Test
	public void test_Upper_Boundary() {
		//crossExcessRate is maximized to 100%
		Parameters.initializeParameterCollections(new String[]{"io:false", "netio:false", "connectToInputs:false", "crossExcessRate:1.0", "mating:true"});
        MMNEAT.loadClasses();
        TWEANNGenotype m = new TWEANNGenotype(MMNEAT.networkInputs, MMNEAT.networkOutputs, true, 1, 1, 0);
        for(int i =0; i < NUM_MUTATION_ITERATIONS; i++) {//performs some mutations on m
        	m.linkMutation();	
        	m.spliceMutation();
        	m.linkMutation();
        	m.weightMutation();;
        }
        TWEANNGenotype f = (TWEANNGenotype) m.copy();//copies m. Does so after some mutation of m so m is an ancestor
        for(int i =0; i < NUM_MUTATION_ITERATIONS; i++) {//performs mutations on both m and f
        	m.linkMutation();	
        	m.spliceMutation();
        	m.linkMutation();
        	m.weightMutation();
		...
		assertTrue(m.numIn == f.numIn && m.neuronsPerMode == f.neuronsPerMode);
		 ...
		 ArrayList<NodeGene> oGenotype = o.nodes;//creates long arrays to facilitate utilization a helper method
		 long[] oInno = new long[oGenotype.size()];
		 for(int i = 0; i < oGenotype.size(); i++) {
			 oInno[i] = oGenotype.get(i).innovation; 
		 }
		 ArrayList<NodeGene> originalFGenotype = originalF.nodes;//creates long arrays to facilitate utilization a helper method
		 ...
		 ArrayList<NodeGene> originialMGenotype = originalM.nodes;//creates long arrays to facilitate utilization a helper method
		 ...
		 for(int i = 0; i < originalFInno.length; i++ ) {//iterates through parent innovation numbers and asserts both offspring have them
			assertTrue(ArrayUtil.member(originalFInno[i], mInno));
			assertTrue(ArrayUtil.member(originalFInno[i], oInno));
		 }
		 for(int i = 0; i < originalMInno.length; i++ ) {//iterates through parent innovation numbers and asserts both offspring have them
	...
	/**
	 * Set crossExcessRate to 0.0 so neither offspring inherits innovation numbers from the other parent.
	 * Essentially assures no crossover occurs.
	 */
	@Test
	public void test_Lower_Boundary() {
		//crossExcessRate is minimized to 0.0%
		Parameters.initializeParameterCollections(new String[]{"io:false", "netio:false", "connectToInputs:false", "crossExcessRate:0.0", "mating:true"});
        MMNEAT.loadClasses();
        TWEANNGenotype m = new TWEANNGenotype(MMNEAT.networkInputs, MMNEAT.networkOutputs, true, 1, 1, 0);
        TWEANNGenotype f = (TWEANNGenotype) m.copy();//assures f has same architecture and structure but not ancestors
        for(int i =0; i < NUM_MUTATION_ITERATIONS; i++) {//mutates both parents iteratively
        	m.linkMutation();	
        	m.spliceMutation();
        	m.linkMutation();
        	m.weightMutation();
		...
		assertTrue(m.numIn == f.numIn && m.neuronsPerMode == f.neuronsPerMode);
		 ...
		 for(int i = 0; i < originalFInno.length; i++ ) {//iterates through parent innovation numbers to assert only one child inherited them
			 if(originalFInno[i] > 0) {
			assertFalse(ArrayUtil.member(originalFInno[i], mInno));
			assertTrue(ArrayUtil.member(originalFInno[i], oInno));
		 }
		 }
		 for(int i = 0; i < originalMInno.length; i++ ) {//iterates through parent innovation numbers to assert only one child inherited them
