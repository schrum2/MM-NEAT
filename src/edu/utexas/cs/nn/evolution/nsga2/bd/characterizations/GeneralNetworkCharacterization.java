...
public class GeneralNetworkCharacterization<T extends Network> implements BehaviorCharacterization<T> {
    ...
    protected ArrayList<double[]> syllabus;//list of inputs to feed into network to get behavior vector
     ...
     * @param syllabusSize size of syllabus
     */
    ...
    protected ArrayList<Double> getBehaviorVector(T net, ArrayList<double[]> syllabus) {
        ...
        for (int x = 0; x < syllabus.size(); x++) {//gets behavior vector by processing syllabus through net
            double[] output = net.process(syllabus.get(x));//uses processed syllabus to compare networks' diversity
    ...
    /**
     * gets a new array list of random inputs to use for behavioral diversity testing 
    ...
    /**
     * creates a new random syllabus  which must be
     * prepped to test the networks
     */
    public void prepare() {
        newRandomSyllabus();
    }
