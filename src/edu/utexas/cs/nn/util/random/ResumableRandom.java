...
/**
 *This class creates a random number generator that stores the current seed value so
 *the program can be resumed with the current seed saved.
 *
 * @author Jacob
 */
public class ResumableRandom extends Random {
	...
	/**
	 * creates a random number generator using the seed parameter, is inherited from java.util
	 * 
 ...
 */
    public void hardSetSeed(long newSeed) {
        try {
            ...
            //resets seed of resumable random number generator to the long parameter newSeed
            seed.set(newSeed);
            //these exceptions should not occur and if so, they crash the program
    ...
    /**
     * Performs same operation as hardSetSeed method to access the seed of the random number
     * generator, but instead returns the seed currently being used for the random number generator.
     * 
    ...
    public long getSeed() {
        ...
        //if the try and catch clauses fail, returns 0. Random number generators never have seed value so client can 
        //know code is broken if 0 is returned. Should never occur theoretically.
        return 0;
    }
