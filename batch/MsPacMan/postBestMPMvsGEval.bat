...
cd ..
cd ..
java -jar "dist/MM-NEATv2.jar" runNumber:%4 experiment:edu.utexas.cs.nn.experiment.post.BestNetworkExperiment base:%1 log:%2-%3 saveTo:%3 trials:%5 watch:false showNetworks:false io:false netio:false onlyWatchPareto:true printFitness:false pacManGainsLives:true pacmanLives:3 animateNetwork:false monitorInputs:false logDeathLocations:false pacManLevelTimeLimit:3000 pacmanMaxLevel:16 pacmanFatalTimeLimit:false evalReport:true timedPacman:true getRemainingPills:true
