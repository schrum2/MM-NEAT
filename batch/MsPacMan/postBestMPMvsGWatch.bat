...
cd ..
cd ..
java -jar "target/MM-NEAT-0.0.1-SNAPSHOT.jar" runNumber:%4 experiment:edu.southwestern.experiment.post.BestNetworkExperiment base:%1 log:%2-%3 saveTo:%3 trials:%5 watch:true showNetworks:true io:false netio:false onlyWatchPareto:true printFitness:false pacManGainsLives:true pacmanLives:3 animateNetwork:false monitorInputs:true logDeathLocations:false pacManLevelTimeLimit:3000 pacmanMaxLevel:16 pacmanFatalTimeLimit:false evalReport:true timedPacman:true getRemainingPills:true modePheremone:true partiallyObservablePacman:true observePacManPO:false
