...
game <- args[1]
#game <- "MarioLevelsDecorateNSLeniency"
...
evolutionData <- data.frame(generation = integer(), type = character(), run = integer(), score = double())
...
for(t in types) {
  ...
  for(d in directories) {
    ...
    temp <- read.table(file = paste("./",d,"/",game,"-",d,"_PercentBeatable.txt", sep = ""), sep = '\t', header = FALSE)
    # Rename relevant column
    colnames(temp)[beatableIndex] <- "Beatable"
    colnames(temp)[unbeatableIndex] <- "Unbeatable"
    colnames(temp)[percentIndex] <- "PercentBeatable"
    
    typeLabel <- t
    # Add beatable data
    #evolutionData <- rbind(evolutionData, data.frame(generation = temp$V1, 
    #                                                 type = paste(typeLabel," Beatable",sep=""),
    #                                                 run = substring(d,nchar(t)+1), # Get the number following the type
    #                                                 score = temp$Beatable))
    # Add unbeatable data
    #evolutionData <- rbind(evolutionData, data.frame(generation = temp$V1, 
    #                                                 type = paste(typeLabel," Unbeatable",sep=""),
    #                                                 run = substring(d,nchar(t)+1), # Get the number following the type
    #                                                 score = temp$Unbeatable))
    # Add percent beatable data
    evolutionData <- rbind(evolutionData, data.frame(generation = temp$V1, 
                                                     type = paste(typeLabel,sep=""),
                                                     run = substring(d,nchar(t)+1), # Get the number following the type
                                                     score = temp$PercentBeatable))
...
print(evolutionStats[(evolutionStats$generation == 999),])

evolutionStats$generation <- evolutionStats$generation * 100

cbPalette <- c("#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")

saveFile <- paste("AVG-PercentBeatable-",game,".pdf",sep="")
